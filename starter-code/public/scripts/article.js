...
// TODO - COMPLETED: Wrap the entire contents of this file in an IIFE.
...
(function (module) {
  ...
  Article.loadAll = rows => {
    ...
    // TODO-COMPLETED: Refactor this forEach code, by using a `.map` call instead, since what we are trying to accomplish
...
// TODO -COMPLETED: Chain together a `map` and a `reduce` call to get a rough count of all words in all articles. -- what goes in map/reduce
Article.numWordsAll = () => {
  return Article.all.map(article => article.body).reduce( function (total, body) {return total + body.split(' ').length}, 0)
};

// TODO -- COMPLETED: Chain together a `map` and a `reduce` call to produce an array of unique author names. You will
// probably need to use the optional accumulator argument in your reduce call. -- generate an array of all author names
Article.allAuthors = () => {
  return Article.all.map(article =>article.author).reduce( function (authorArray, author) {
  ...
  return Article.allAuthors().map(author => {
    return {
      ...
      numWords: Article.all.filter(article => article.author == author)
        ...
        .reduce(function (total, body) {return total + body.split(' ').length}, 0)
    }
    // TODO - COMPLETE: Transform each author string into an object with properties for
